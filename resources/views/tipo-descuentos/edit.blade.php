...
<div class="container-fluid px-4">
    <h1 class="mt-4">Editar Tipo de Descuento</h1>
    ...
    <div class="card shadow mb-4">
        <div class="card-header">
            <i class="fa-solid fa-percent me-1"></i> Editar Descuento
        </div>
        <div class="card-body">
            <form action="{{ route('tipo-descuentos.update', $tipoDescuento->id) }}" method="POST">
                @csrf
                @method('PUT')

                <div class="mb-3">
                    <label for="tipo_descuento" class="form-label">Nombre del Descuento</label>
                    <input type="text" class="form-control" id="tipo_descuento" name="tipo_descuento" value="{{ old('tipo_descuento', $tipoDescuento->tipo_descuento) }}" required>
                </div>

                <div class="mb-3">
                    <label for="porcentaje" class="form-label">Porcentaje (%)</label>
                    <input type="number" class="form-control" id="porcentaje" name="porcentaje" value="{{ old('porcentaje', $tipoDescuento->porcentaje) }}" step="0.01" min="0" max="100" required>
                </div>
